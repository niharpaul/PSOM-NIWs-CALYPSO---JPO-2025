subroutine write_cdf_2D_x(islice,counter_2d,n)
  ...
  !---- ------------------------------------------------------------      
  USE header,ONLY : NI,NJ,NK,ntr,s,T,p,rho,u,v,w,consump,Tr,shear,vor,freqN2,yc,zc,&
  & fricb,stress_top_x,DL,time_seconds,dirout,rc_kind, pvt,rp, stress_top_y, pv3
  !     reads in a netcdf file and interpolates the data from the sigma grid
  !     onto a z level                                                    
  include 'netcdf.inc'                                                                        
  ...
  REAL(kind=rc_kind) :: trbar, wbar, wtrbar
  !,fbmean 
  ...
  REAL(kind=4)   :: yslice(NJ),zslice(NJ,NK),conslice(ntr,NJ,NK),       &
	   ...
	   ugslice(NJ,NK),vgslice(NJ,NK),                               &
           ...
           timday, pvtslice(NJ,NK), pv3slice(NJ,NK), pslice(NJ,NK),     &
           by(NJ,NK) 
...
!     &     stressxsp(NJ), stressysp(NJ) 
 
   REAL (kind=rc_kind) :: stressxsp(NJ), stressysp(NJ)	                                                                       
   REAL (kind=rc_kind) :: byslice(NJ,NK), bysection(NJ,NK),             &
      &     bzslice(NJ,NK), trbarsl(ntr,NJ,NK), wbarsl(NJ,NK),          &
      &     wtrbarsl(ntr,NJ,NK)
 ! ,&  &   psivslice(NJ,NK),                                            &
 !   &     psiwslice(NJ,NK),vbbarsl(NJ,NK),wbbarsl(NJ,NK),              &
 !   &     psieddysl(NJ,NK),rhobarslice(NJ,NK),                         &
 !   &     wpvbarsl(NJ,NK),pvbarsl(NJ,NK),
 ...
 !    &     wcbarsl(ntr,NJ,NK),csfeddysl(ntr,NJ,NK)                      
  ...
  integer :: idvstrain,idvstressx,idvstressy,idvstr,idvs,idvtbarsl,idvtemp,idvtim,idvtr,idvt,idvu,idvvb,idvvc,idvvor,idvv, idvug, idvvg
  integer :: idvwb,idvwc,idvwpv,idvw,idvwbarsl,idvwtrbarsl,idvy,idvzsave,idvz,idwdx,idwdy,idwdz,iimday,ipos
  integer :: idvzave,idvshear, idvpvt, idvpv3
  REAL(kind=rc_kind) :: rcode

  timday=time_seconds/86400.d0
        ...
        do j=1,NJ 
                do itr=1,ntr 
                        ...
                        do i=1,NI 
                                ...
                                wtrbar=wtrbar+ w(i,j,k,n)*Tr(itr,i,j,k,n)
                        end do 
                        trbarsl(itr,j,k)  = trbar/NI 
                        wtrbarsl(itr,j,k) = wtrbar/NI
                end do 
                wbar=0d0
                do i=1,NI
                        wbar=wbar+w(i,j,k,n)
...
!     write to a netcdf file                  
      ...
      if (counter_2d.eq.1) then 
         ...
         idvstressx = ncvdef(idDatFile,'stressx', NCDOUBLE,2,dimswind,rcode) 
         idvstressy = ncvdef(idDatFile,'stressy', NCDOUBLE,2,dimswind,rcode) 
         !idvcon    = ncvdef(idDatFile,'consump', NCFLOAT, 4,dims4,rcode) 
         idvtr      = ncvdef(idDatFile,'tr',      NCFLOAT, 4,dims4,rcode) 
         idvtbarsl  = ncvdef(idDatFile,'trbarsl', NCDOUBLE,4,dims4,rcode)
	 idvwtrbarsl= ncvdef(idDatFile,'wtrbarsl',NCDOUBLE,4,dims4,rcode) 
         ...
         idvug  = ncvdef(idDatFile,'ug',NCFLOAT,3,dims,rcode) 
         idvv   = ncvdef(idDatFile,'v' ,NCFLOAT,3,dims,rcode) 
         idvvg  = ncvdef(idDatFile,'vg',NCFLOAT,3,dims,rcode) 
         idvw   = ncvdef(idDatFile,'w' ,NCFLOAT,3,dims,rcode)
	 idvwbarsl=ncvdef(idDatFile,'wbarsl',NCDOUBLE,3,dims,rcode) 
         ...
         idvpvt = ncvdef(idDatFile,'pvt',NCFLOAT,3,dims,rcode) 
         idvpv3 = ncvdef(idDatFile,'pv3',NCFLOAT,3,dims,rcode) 
         !idvp   = ncvdef(idDatFile,'pre',NCFLOAT,3,dims,rcode)
         ...
         !idvpec = ncvdef(idDatFile,'psieddytracer',NCFLOAT,4,dims4,rcode)                                             
...
!         call ncredf(idDatFile,rcode)                                  
         ...
         idvtbarsl  = NCVID(idDatFile, 'trbarsl', RCODE) 
         ...
         idvwbarsl  = NCVID(idDatFile, 'wbarsl',  RCODE)
         idvwtrbarsl= NCVID(idDatFile, 'wtrbarsl',RCODE)
	 ...
	 !print *, RCODE 
      ...
      CALL ncvpt(idDatFile,idvtbarsl, start4,count4,Trbarsl,   rcode)
      CALL ncvpt(idDatFile,idvwtrbarsl,start4,count4,wtrbarsl, rcode) 
      ...
      CALL ncvpt(idDatFile,idvwbarsl, start, count, wbarsl,    rcode) 
